...
static Value _helloWorld(Runtime &rt, TurboModule &turboModule,
	...
	auto callback = std::make_shared<Value>((args[0].asObject(rt)));
	
	std::thread th { [&rt, callInvoker, callback](){
		std::this_thread::sleep_for(std::chrono::milliseconds(1000));
		callInvoker->invokeAsync([callback, &rt]() {
			string helloworld = "helloworld";
			callback->asObject(rt).asFunction(rt).call(rt, String::createFromUtf8(rt, helloworld));
		});
	}};
